...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => this.renderPathTable());
} else {
    ...
    setupListeners() {
        this.worker.onmessage = (e) => {
            ...
            if (data.loaded) {
                ...
                this.requestStats();
                this.requestExistingPaths(); // <-- new: check which paths exist
...
cell.classList.add(done ? 'status-found' : 'status-missing');
    ...
    generateSchema() {
        ...
        try {
            ...
            this.showToast('Schema generation failed', 'error');
        }
    }
